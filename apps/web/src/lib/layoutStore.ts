...
export function setLayerPanelHeaderHeight(height: number): void {
  const next = Number.isFinite(height) ? Math.max(0, height) : DEFAULT_LAYER_HEADER_HEIGHT;
  _layerPanelHeaderHeight.set(next);
}

export function setLayerPanelFirstRowOffset(offset: number): void {
  const next = Number.isFinite(offset) ? Math.max(0, offset) : DEFAULT_LAYER_FIRST_ROW_OFFSET;
